...
(check-part 'a .2 (top Q2) is 1)
(check-part 'b .6 (top Q3) is 0)
(check-part 'c .2 (top Q4) is 1)

(exercițiul 6 : 20 puncte)
(when (andmap struct? (list C1 C2 C3 C4 C5))
  (check-part 'a .0125 (list (counter-index C1) (counter-tt C1) (counter-et C1) (counter-queue C1)) is (list 1 0 0 empty-queue))
  (check-part 'b .0125 (list (counter-index C4) (counter-tt C4) (counter-et C4) (counter-queue C4)) is (list 4 0 0 empty-queue))
  (check-part 'c .025 (map counter-tt (list C1 C2 C3 C4 C5)) is '(0 0 0 0 12))
  (check-part 'd .025 (map counter-et (list C1 C2 C3 C4 C5)) is '(0 0 0 0 8))
  (check-part 'e .175 (map counter-queue (list C1 C2 C3 C4 C5)) is
              (list empty-queue empty-queue empty-queue empty-queue
                    (queue '((remus . 6) (vivi . 4)) '() 2 0)))
  )
(when (andmap struct? (list C1))
  (check-part 'f .0625 ((add-to-counter 'ana 10) C1) is
              (counter 1 10 10 (queue '() '((ana . 10)) 0 1)))
  (check-part 'g .0625 ((add-to-counter 'bogdan 1) (counter 1 2 2 (queue '((ana . 2)) '() 1 0))) is
              (counter 1 3 2 (queue '((ana . 2)) '((bogdan . 1)) 1 1)))
  (check-part 'h .0625 ((add-to-counter 'cristi 3) (counter 1 2 2 (queue '() '((ana . 2)) 0 1))) is
              (counter 1 5 2 (queue '() '((cristi . 3) (ana . 2)) 0 2)))
  (check-part 'i .0625 ((add-to-counter 'dana 4) (counter 1 6 3 (queue '((ana . 2)) '((cristi . 2) (bogdan . 1)) 1 2))) is
              (counter 1 10 3 (queue '((ana . 2)) '((dana . 4) (cristi . 2) (bogdan . 1)) 1 3)))
  )
(check-part 'j .1 (remove-first-from-counter (counter 3 2 2 (queue '((mara . 2)) '() 1 0))) is
            (counter 3 0 0 empty-queue))
(check-part 'k .1 (remove-first-from-counter (counter 1 11 11 (queue '() '((mara . 7)) 0 1))) is
            (counter 1 0 0 empty-queue))
(check-part 'l .1 (remove-first-from-counter (counter 2 113 100 (queue '((ana . 100) (dan . 4) (gigi . 9)) '() 3 0))) is
            (counter 2 13 4 (queue '((dan . 4) (gigi . 9)) '() 2 0)))
(check-part 'm .1 (remove-first-from-counter (counter 4 42 22 (queue '() '((dan . 9) (gigi . 10) (ana . 1) (mara . 12)) 0 4))) is
            (counter 4 20 1 (queue '((ana . 1) (gigi . 10) (dan . 9)) '() 3 0)))
(check-part 'n .1 (remove-first-from-counter (counter 7 69 37 (queue '((irina . 10)) '((dan . 9) (gigi . 10) (ana . 1) (mara . 12)) 1 4))) is
            (counter 7 32 12 (queue '() '((dan . 9) (gigi . 10) (ana . 1) (mara . 12)) 0 4)))
...
(when (andmap struct? (list C1 C2 C3 C4))
              ...
              (serve '(3 (ana 13) 2 (mia 2) 1 (mara 1) 1 (ensure 3) (ion 5) 2 (gigi 6) 4
                     ...
                     (list C3)) is
                                (list
                                 ...
                                 (counter 6 0 0 (queue '() '() 0 0))))
  (check-part 'k (/ 1. 20)
              (serve '(10 (delay 5 11) (delay 4 5) (delay 4 3) (delay 2 7) (isa 9) 2 (geo 10) )
                     (list (empty-counter 1) (empty-counter 2) (empty-counter 3))
                     (list (empty-counter 4) (empty-counter 5)))
              is (list
                  '()
                  (empty-counter 1)
                  (counter 2 5 5 (queue '() '() 0 0))
                  (empty-counter 3)
                  (counter 4 15 15 (queue '() '((isa . 9)) 0 1))
                  (counter 5 19 19 (queue '() '((geo . 10)) 0 1))
                  ))
  (check-part 'l (/ 1. 20)
              (serve '(5 (leo 5) (mia 6) (meda 4) (delay 1 3) 8 (delay 5 30) (delay 2 20) 10 )
                     (list (empty-counter 1) (empty-counter 2))
                     (list (empty-counter 3) (empty-counter 4) (empty-counter 5)))
              is (list
                  '((2 . meda)(3 . mia)(1 . leo))
                  (empty-counter 1)
                  (counter 2 10 10 (queue '() '() 0 0))
                  (empty-counter 3)
                  (empty-counter 4)
                  (counter 5 20 20 (queue '() '() 0 0))
                  ))
  (check-part 'm (/ 1. 20)
              (serve '(30 22 36 (teo 2) (eliza 9) 6 (ina 20) 6 (ensure 12) (delay 2 12) (horia 13) )
                     (list (empty-counter 1) (empty-counter 2))
                     (list (empty-counter 3)))
              is (list
                  '((1 . teo)(3 . eliza))
                  (empty-counter 1)
                  (counter 2 12 12 (queue '() '() 0 0))
                  (counter 3 30 17 (queue '((ina . 20)) '((horia . 13)) 1 1))
                  ))
  (check-part 'n (/ 1. 20)
              (serve '((bia 7) 39 (ensure 12) (roua 19) (delay 4 19) (cezar 7) (elian 8) (ensure 9) (delay 3 34) (geo 15) (ada 5) )
                     (list (empty-counter 1) (empty-counter 2))
                     (list (empty-counter 3) (empty-counter 4)))
              is (list
                  '((3 . bia))
                  (counter 1 5 5 (queue '() '((ada . 5)) 0 1))
                  (empty-counter 2)
                  (counter 3 60 53 (queue '() '((cezar . 7) (roua . 19)) 0 2))
                  (counter 4 27 27 (queue '() '((elian . 8)) 0 1))
                  (counter 5 15 15 (queue '() '((geo . 15)) 0 1))
                  (empty-counter 6)
                  ))
  (check-part 'o (/ 1. 20)
              (serve '((delay 2 46) (ensure 6) (delay 1 4) (delay 5 3) (delay 3 19) 15 (noa 13) (ensure 6) (ensure 6) (edi 12) (ensure 9) 33 )
                     (list (empty-counter 1) (empty-counter 2) (empty-counter 3))
                     (list (empty-counter 4)))
              is (list
                  '((5 . edi)(4 . noa))
                  (empty-counter 1)
                  (empty-counter 2)
  ...
  (check-part 'p (/ 1. 20)
              (serve '(34 22 (delay 4 37) (leo 5) (delay 3 24) (jack 11) 21 46 )
                     (list (empty-counter 1) (empty-counter 2))
                     (list (empty-counter 3) (empty-counter 4)))
              is (list
                  '((1 . leo)(3 . jack))
  ...
  (check-part 'q (/ 1. 20)
              (serve '((delay 4 18) (paul 4) (cristi 1) 6 (delay 1 30) (delay 1 12) (delay 5 9) (ensure 7) )
                     (list (empty-counter 1) (empty-counter 2) (empty-counter 3))
                     (list (empty-counter 4) (empty-counter 5)))
              is (list
                  '((2 . cristi)(1 . paul))
                  (counter 1 42 42 (queue '() '() 0 0))
                  (empty-counter 2)
                  (empty-counter 3)
                  (counter 4 12 12 (queue '() '() 0 0))
                  (counter 5 9 9 (queue '() '() 0 0))
                  (empty-counter 6)
                  (empty-counter 7)
                  (empty-counter 8)
                  (empty-counter 9)
  ...
  (check-part 'r (/ 1. 20)
              (serve '((petra 7) (delay 4 3) (teo 4) (noa 10) 3 (cati 9) 20 )
                     (list (empty-counter 1) (empty-counter 2) (empty-counter 3))
                     (list (empty-counter 4)))
              is (list
                  '((1 . teo)(4 . petra)(4 . noa))
                  ...
                  (empty-counter 2)
                  (empty-counter 3)
                  (counter 4 6 6 (queue '((cati . 9)) '() 1 0))
                  ))
  (check-part 's (/ 1. 20)
              (serve '(32 (alice 9) (bia 7) (ensure 11) (octav 14) (ensure 13) (hana 20) (nico 2) (ensure 13) )
                     (list (empty-counter 1) (empty-counter 2))
                     (list (empty-counter 3) (empty-counter 4)))
              is (list
                  '()
                  (counter 1 2 2 (queue '() '((nico . 2)) 0 1))
                  (empty-counter 2)
                  (counter 3 29 9 (queue '() '((hana . 20) (alice . 9)) 0 2))
                  (counter 4 21 7 (queue '() '((octav . 14) (bia . 7)) 0 2))
                  ))
  (check-part 't (/ 1. 20)
              (serve '(1 (paul 12) (liam 15) (lara 16) (delay 5 46) 25 )
                     (list (empty-counter 1) (empty-counter 2) (empty-counter 3))
                     (list (empty-counter 4) (empty-counter 5)))
              is (list
                  '((4 . paul))
                  ...
                  (empty-counter 2)
                  (empty-counter 3)
                  (counter 4 3 3 (queue '((lara . 16)) '() 1 0))
                  (counter 5 36 36 (queue '() '((liam . 15)) 0 1))
                  ))
  )
